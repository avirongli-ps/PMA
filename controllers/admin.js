...
/*
    1. Get user, project and activity count
...
exports.getDashboard = async(req, res, next) => {
    ...
    try{
        ...
        const projects_count = await Project.find().countDocuments();
...
exports.postDashboard = async(req, res, next) => {
    try {
        ...
        // getting user and project count
        const projects_count = await Project.find().countDocuments();
...
// admin -> get project inventory working procedure
/*
    1. Construct search object
    2. Fetch projects by search object
    3. Render admin/projectInventory
*/
exports.getAdminProjectInventory = async(req, res, next) => {
    try{
        ...
        const projects_count = await Project.find(searchObj).countDocuments();
        ...
        res.render("admin/projectInventory", {
            projects : projects,
            current : page,
            pages: Math.ceil(projects_count / PER_PAGE),
            filter : filter,
            value : value,
        });
    } catch(err) {
...
*/
exports.postAdminProjectInventory = async(req, res, next) => {
    try {
        let page = req.params.page || 1;
        ...
        // fetch the projects by search query
        const projects = await Project
            .find(searchObj)
            .skip((PER_PAGE * page) - PER_PAGE)
            .limit(PER_PAGE);
...
// admin -> get the project to be updated
...
exports.postUpdateProject = async(req, res, next) => {
    ...
    try {
        ...
        res.redirect("/admin/projectInventory/all/all/1");
    } catch (err) {
        console.log(err);
        res.redirect('back');
    }
...
exports.getDeleteProject = async(req, res, next) => {
    try {
        ...
        req.flash("success", `A project named ${project.title} is just deleted!`);
        res.redirect('back');
